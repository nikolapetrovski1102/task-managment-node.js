...
const FormDisabledDemo = () => {
  ...
  const showDeleteConfirm = () => {
    confirm({
      ...
      content: `Are your sure you want to delete ${ taskName }?`,
      okText: 'Yes',
      okType: 'danger',
      cancelText: 'No',
      async onOk() {
...
const saveTaskPost = async () => {
  ...
  >
    ...
    <div style={{ display: 'flex', gap: '8px', marginRight: '1%' }}>
          <Button id='edit_btn' type="primary" onClick={ () => navigate(`/task/edit/${id}`)}>
            Edit
          </Button>
          <Button id='delete_btn' onClick={ showDeleteConfirm } type="dashed" danger>
            Delete
          </Button>
        </div>
      </div>
    ...
    >
      ...
      >
        <h1 style={{ textAlign: 'left', marginLeft: '4.5%', marginTop: '1%'}} >
            {taskName}
        </h1>
        <Divider />
        ...
        <Form.Item style={{ width: '150%', marginLeft: '-17%' }} label="Description" >
          <TextArea 
            disabled={true} 
            value={ description } 
            rows={10}
