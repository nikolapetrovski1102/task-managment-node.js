  ...
  useEffect(() => {
    ...
    axios.get(`${process.env.REACT_APP_API_URL}/completed/tasks`, {
      ...
      const tasks = response.data.map((task) => ({
        ...
        key: task.id,
        taskName: task.task_id.name,
        dueDate: task.task_id.dueDate != null ? task.task_id.dueDate.split('T')[0] : '',
        finishedAt: task.finished_at.split('T')[0],
        createdAt: task.created_at,
        project:  <p style={{ marginBottom: '0' }} > <Avatar style={{ marginBottom: '1px' }} size={'small'} src={getProject(task.task_id.project)} ></Avatar> {task.task_id.project} </p>,
      }));
    ...
    <Spin spinning={loading} size='large' tip='Loading data..'>
      <Table
        ...
        onRow={(record) => {
          return {
            onDoubleClick: () => {
              navigate(`/task/details/${record.task_id}`);
            },
            onClick: () => {
