...
import UserTableDetails from './UserTableDetailsFrom.tsx';
...
const App =  ({ userId })  => {
  ...
  const items2 = [
    ...
    },
    {
      ...
      label: 'Team',
      children: [
        ...users.map(user => ({ key: `user-${user.id}`, label: localStorage.getItem('user_role') == 'User' ? <p>{user.fullname}</p> : <a onClick={ handleRedirect(user) } >{user.fullname}</a> }))
      ],
    },
  ];
